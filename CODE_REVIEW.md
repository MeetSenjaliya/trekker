...
- [x] Drop the unused `mood` enum and the dead `increment_participants` RPC (Phase 0 NEW-5).
      **Keep `update_participants_count()`** — it backs the `participants_joined` counter.
      `increment_participants` went 2026-08-08 (FEATURES.md NEW-5); `mood` dropped **2026-08-12** via
      plain `drop type public.mood` (no `cascade`, so Postgres would have refused had anything depended
      on it). Verified live over read-only MCP: `public` now holds exactly six enums — `account_type`,
      `company_role`, `company_status`, `difficulty`, `experience_level`, `gender`. `schema.sql` and
