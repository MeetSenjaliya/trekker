...
export default function ExploreClient({ initialData }: { initialData: SearchTreksResult }) {
  ...
  // The server rendered exactly this query (defaults, page 1), so hand its rows
  // to TanStack Query as initialData instead of refetching them on mount. Any
  // other filter set is a different object identity and falls through to a fetch.
  const isServerRenderedQuery = currentPage === 1 && debouncedFilters === DEFAULT_FILTERS;

  const { data, isFetching } = useSearchTreks(
    debouncedFilters,
    currentPage,
    TREKS_PER_PAGE,
    isServerRenderedQuery ? initialData : undefined
  );
              ...
              return (
                <div key={trek.id} className="transform hover:scale-[1.02] transition-transform duration-300">
                  <TrekCard
                    ...
                    companyName={trek.company_name}
                    companySlug={trek.company_slug}
                  />
                </div>
              );
            })}
