...
_Last updated: 2026-09-21 — full history in [§3 Changelog](#3--changelog-newest-first)._
...
|---|---------|----------------|--------|
| 0 | **Finish the Supabase Storage → Cloudflare R2 move — Parts 2 and 3** of [`supabase to cloudflare trasformation.md`](supabase%20to%20cloudflare%20trasformation.md). Part 1 (code + `0030`) is done and green; production still serves every image from Supabase | **Part 2 (go live, reversible):** create the `trekker-media` bucket + r2.dev public access + a bucket-scoped Object Read & Write token in the Cloudflare dashboard (the MCP token can only read R2 — `10000: Authentication error` on create); set the five `R2_*` vars in `.env.local` and Vercel; `SUPABASE_SECRET_KEY=… node --env-file=.env.local scripts/migrate-storage-to-r2.mjs` until every bucket reports 0 copied; apply `0030`; deploy. New uploads then go to R2 while old URLs still render from Supabase (both hosts are in `remotePatterns` and CSP `img-src`). **Part 3 (cutover):** paste `scripts/rewrite-storage-urls.sql` (counts must be 0); write + apply `0032` (drop the storage trigger, `upload_rate_limited`, the 14 `storage.objects` policies); delete `tests/db/storage-{writes,listing,content-type}.test.ts`, drop `enforce_storage_rate_limit` from `acl.test.ts`'s list; after a few clean days delete the five Supabase buckets and remove the Supabase host from `next.config.mjs` / `img-src`. Until Part 1 is deployed the default images (`src/lib/defaultImages.ts`) point at an R2 URL that has no objects yet — they 404 on a preview, by design | [§2](#image-uploads-move-to-cloudflare-r2--part-1-route-rules-rpc-and-scripts-2026-09-21) |
| 0b | **Refund the upload slot when the R2 `PUT` fails** — a new `refund_upload()` SECURITY DEFINER RPC (`0033`, after the Part-3 `0032`) that deletes the caller's own just-inserted `rate_events` row, called from [`route.ts`](src/app/api/upload/route.ts) in the two R2-failure branches | `record_upload()` counts **before** the write on purpose: check-and-insert under one advisory lock is what keeps two simultaneous uploads from both passing the cap. So an R2 failure (outage, bad credentials) burns one of the 6/hour anyway — seen 2026-09-21 when the missing `Content-Length` bug cost the owner two slots. Counting *after* the write instead would let 50 concurrent requests all pass before any is recorded — not an option. Decided 2026-09-21: architecture is right, refund later; a lost slot on a genuine R2 failure is an annoyance, not a hole | [§2](#image-uploads-move-to-cloudflare-r2--part-1-route-rules-rpc-and-scripts-2026-09-21) |
| 0c | **Apply [`0031_cap-review-photos-at-five.sql`](supabase/migrations/0031_cap-review-photos-at-five.sql)** in the SQL Editor, then confirm the ledger row | `CHECK (cardinality(photo_urls) <= 5)` on `trek_reviews`. The form has always refused a sixth photo, but a direct API write never had to use the form — and the `shutterbug` badge sums exactly this count. Live data checked 2026-09-21 over MCP: 2 reviews, at most 2 photos, so it validates in place. Until it is applied, the five-photo cap is client-only (as it always was) | [§2](#reviews-go-live-on-the-trek-page-photos-through-the-r2-route-2026-09-21) |
...
> re-evaluated on the live server rather than trusted from PGlite, and the
> `\d`/`\s` ARE trap `0013` found is absent: `'dsdsds'` fails the phone rule,
> so the classes are being read as classes. See
> [§3](#company-website-urls-are-pinned-to-https--2026-09-05) and
> [§3](#the-companies-table-gets-the-caps-0009-and-0011-skipped--2026-09-05).
>
...
## Reviews go live on the trek page, photos through the R2 route (2026-09-21)

**Status:** 🟡 — code complete and green (`npm run build`, `npm test` 436). `0031` written and proven in PGlite, **not applied** (§1.0 row 0c). Not deployed.

- **What was actually missing.** `FEATURES.md` marked "Reviews (submit + showcase)" ✅, but nothing in `src/` ever inserted into `trek_reviews`: [`src/app/(trekker)/review/page.tsx`](src/app/(trekker)/review/page.tsx) is a mockup with hardcoded reviews and a `ReviewForm` whose `onSubmit` it never passes (already flagged in §3, 2026-09-09). So "review photos to R2" meant building the submission. The upload half was already there — the route accepts `kind: 'review'`, keys the object under the caller's own folder (`trek-reviews/{uid}/…`), and `record_upload('trek-reviews')` counts it on its own 20/hour counter (`0030`).
- **Where the form lives.** The Reviews section of [`src/app/trek/[id]/TrekDetailClient.tsx`](src/app/trek/[id]/TrekDetailClient.tsx), behind a "Write a review" toggle that renders only when the signed-in trekker holds a **confirmed** booking on a departure that has **ended** and has not reviewed this trek yet. Eligibility comes from the `trek_participants` rows the page already fetches, with the end day computed by `lastTrekDay()` in [`src/lib/reviews.ts`](src/lib/reviews.ts) — a deliberate copy of the `0018` formula (`batch_date + (greatest(1, ceil(duration_hours/24)) − 1)`), pinned by [`src/lib/reviews.test.ts`](src/lib/reviews.test.ts) against the same cases as `tests/db/review-gate.test.ts`. The policy is still what refuses; the copy only decides whether to offer the form. `TrekReview` gains `user_id` (already public — `trek_reviews` is `select … to public`).
- **`submitReview()`** ([`src/lib/reviews.ts`](src/lib/reviews.ts), same `{ success, message }` shape as `joinTrekBatchAndChat`): uploads the photos one at a time through `uploadImage({ kind: 'review' })`, then inserts the row with `photo_urls`. Photos first, row second: a failed upload costs nothing, a refused insert leaves at most five unreferenced immutable objects in R2 — parity with the old avatar/cover uploads, which never deleted their predecessors. Error codes map to plain sentences: `23505` "already reviewed", `42501` "once a confirmed booking of yours has finished", an `UploadError` (429 / 413 / 415) shows the route's own message. `ReviewForm` now awaits `onSubmit` and **keeps the draft** on failure (it used to reset and toast success regardless), and `router.refresh()` re-renders the server-fetched reviews on success.
- **The cap that was form-only.** [`0031_cap-review-photos-at-five.sql`](supabase/migrations/0031_cap-review-photos-at-five.sql): `CHECK (cardinality(photo_urls) <= 5)` on `trek_reviews`. 3 new cases in [`tests/db/review-gate.test.ts`](tests/db/review-gate.test.ts) §6 (five accepted, six refused on insert, five grown to six refused on update). `schema.sql` regenerated (31 migrations).
- **Rate limits that apply, unchanged:** 20 review photos/hour/user (`upload:review`, separate from the 6/hour on avatars/logos/covers); 3 MiB and JPEG/PNG/WebP per file at the route; 5 photos and 2 MB each in the form (the 2 MB is checked *after* `compressImage()`, which targets 1 MB). One review per (trek, user) is the `UNIQUE`, so there is no separate review-insert rate limit to add.
- **Out of scope / follow-ups:** `photo_urls` accepts any string — a direct API write could store a third-party URL; CSP `img-src` stops it loading, and the R2 prefix is environment data so a DB check would be wrong on preview. The `/review` mockup page stays as is (nothing links to it with a trek id). `ReviewForm` is a white light-theme card on the dark trek page — functional, not restyled. No edit/delete UI for an existing review yet (the policies allow both).

## Image uploads move to Cloudflare R2 — Part 1: route, rules, RPC and scripts (2026-09-21)

**Status:** 🟡 — code complete and green (`npm run build`, `npm run lint` 0 errors, 429 tests). `0030` **applied 2026-09-21 08:43 UTC** (ledger row present, `record_upload` read back over MCP); the `trekker` bucket exists with r2.dev access and a local avatar upload succeeds end-to-end (Phase C, 2026-09-21); **not deployed** — production still writes to and reads from Supabase Storage. Parts 2 and 3 are row #0 in §1.0. Plan: [`supabase to cloudflare trasformation.md`](supabase%20to%20cloudflare%20trasformation.md).

- **Why a route handler.** Uploads used to be three browser-side `supabase.storage.from(bucket).upload()` calls with authorization in `storage.objects` RLS and the rate limit in a trigger that storage-api could not report (§13.5 — the `500 {}` problem). R2 has neither, so one Next.js route on Vercel — [`src/app/api/upload/route.ts`](src/app/api/upload/route.ts), `POST` only — now owns the whole decision: `getUser()` → 401; zod-parsed `kind` / `companyId` / `trekId` (UUIDs) → 400; > 3 MiB → 413; **magic-byte sniff** (JPEG / PNG / WebP, which Supabase never could — see `storage-content-type.test.ts`) → 415; `authorizeUpload()` → 403; `record_upload()` false → 429; signed `PUT` via `aws4fetch` with `Cache-Control: immutable` **and an explicit `Content-Length`** → 500 on failure, else `{ url }`. The length header is load-bearing: Next's `fetch` streams a body over a few hundred KB chunked, and R2 answers `411 MissingContentLength` — found 2026-09-21 on the first real avatar upload (a 70-byte PNG passed, a 2 MB JPEG failed); a plain Node script sets the header itself, which is why the credentials tested fine outside the route. It runs **as the signed-in user** over the cookie session (`src/utils/supabase/server.ts`) — no service key — so the company checks are the very RPCs the policies called: `is_company_member && is_company_writable` for `company-logos`, `is_approved_company_member` for `trek-images`; avatars and review photos are keyed to the caller's own folder. Bytes go through the route rather than a presigned URL: files are compressed to ~1 MB client-side and capped at 3 MiB, well under Vercel's 4.5 MB body limit, and the R2 credentials never leave the server.
- **Server-named objects.** [`src/lib/uploadRules.ts`](src/lib/uploadRules.ts) builds the key as `{bucket}/{prefix}/{ts}-{6 hex}.{ext}` — same bucket names and prefixes as today, so copied and new objects share one layout and the Part-3 rewrite is a prefix `replace()`. The client's filename is never used and the extension comes from the sniffed type; this closes the `sanitizeFileName` `..`/empty-string gap, and `sanitizeFileName()` is deleted (nothing else called it).
- **Rate limit as an RPC.** [`0030_record-upload-rpc-for-the-r2-upload-route.sql`](supabase/migrations/0030_record-upload-rpc-for-the-r2-upload-route.sql): `record_upload(p_bucket)` SECURITY DEFINER, `authenticated` only — advisory lock per (action, user) like the `0012` email trigger, count the trailing hour in `rate_events` against the existing `storage_rate_rule()`, insert-and-`true` or `false`. Same 6/hour shared across `avatars`/`company-logos`/`trek-images`, same 20/hour for `trek-reviews`, same zero-policy zero-grant counter. **Additive** — the storage trigger, its 14 policies and the `upload_rate_limited` probe stay until the Part-3 migration (`0032`; `0031` is the review photo cap).
- **Client.** [`src/lib/upload.ts`](src/lib/upload.ts) `uploadImage(file, { kind, companyId?, trekId? })` replaces the three inline blocks in `profile/edit/page.tsx` (`avatar`), `TrekForm.tsx` (`trek-cover`) and `dashboard/settings/page.tsx` (`company-logo` / `company-cover`); `compressImage()` stays where it was. [`src/lib/uploadErrors.ts`](src/lib/uploadErrors.ts) is now a synchronous code → message map — the `upload_rate_limited` round trip and the Supabase status/regex matching are gone. `UploadError` unchanged.
- **Host plumbing.** `src/utils/supabase/middleware.ts` answers an unauthenticated `/api/*` with `401 { error: 'unauthenticated' }` instead of the login redirect a `fetch()` would follow into a 200 HTML page. `next.config.mjs` adds the R2 host to `remotePatterns` and `src/utils/csp.ts` adds it to `img-src` — **Supabase stays in both until Part 3**. The five hard-coded `…supabase.co/storage/…` default-image literals collapse into [`src/lib/defaultImages.ts`](src/lib/defaultImages.ts) (client-safe; `site.ts` is server-only) built from `NEXT_PUBLIC_R2_PUBLIC_URL`. Five new env vars in `.env.local.example`.
- **One-offs for Parts 2/3.** [`scripts/migrate-storage-to-r2.mjs`](scripts/migrate-storage-to-r2.mjs) — idempotent 1:1 key copy of all five buckets (HEAD-skips existing keys, keeps `Content-Type`, sets the immutable cache header; SECRET key on the command line, never stored). [`scripts/rewrite-storage-urls.sql`](scripts/rewrite-storage-urls.sql) — the prefix swap over `profiles.avatar_url`, `treks.cover_image_url`, `companies.logo_url` / `cover_image_url`, `trek_reviews.photo_urls`, ending in per-column counts that must read 0. Not a migration: environment data.
- **Tests.** [`src/lib/uploadRules.test.ts`](src/lib/uploadRules.test.ts) (13): every kind's key layout, the sniff on the three formats and on SVG / HTML / GIF / RIFF-WAVE / truncated buffers, every rule's allow/deny per RPC answer with the RPC injected (the `company.test.ts` pattern), and that the logo rule cannot leak into the trek-cover decision. [`tests/db/upload-rate.test.ts`](tests/db/upload-rate.test.ts) (9): 6 then refuse, exactly 6 rows written, shared counter, `trek-reviews`' own 20, per-user, trailing hour only, no-rule bucket writes nothing, anon `permission denied`, grants exact. The fixtures' seeded `storage.objects` rows still fire the old trigger, so each test clears the actor's counter first (rolled back). The three `storage-*.test.ts` files and `acl.test.ts`'s definer list are **untouched** — they describe policies that are still live. `schema.sql` regenerated (30 migrations).
- **Out of scope, parity with today:** the previous avatar/logo/cover is not deleted on re-upload; ~~`ReviewForm` still never uploads (`kind: 'review'` is accepted by the route)~~ — wired the same day, see [Reviews go live](#reviews-go-live-on-the-trek-page-photos-through-the-r2-route-2026-09-21); `trekId ∈ companyId` is not verified on trek covers (the key is server-built, so nothing escapes the company prefix).
...
| Reviews (submit + showcase) | ✅ | Submit: `src/app/trek/[id]/TrekDetailClient.tsx` → `src/lib/reviews.ts` (photos via `uploadImage({ kind: 'review' })` → R2, then the `trek_reviews` insert under the `0018` gate). Showcase: `ReviewCard` on the trek page. `src/app/(trekker)/review/page.tsx` is still a mockup — see [Reviews go live](#reviews-go-live-on-the-trek-page-photos-through-the-r2-route-2026-09-21) |
...
## Reviews can actually be posted — from the trek page, photos on R2, `0031` caps them at five  ·  2026-09-21

"Add review photos to R2" turned out to be "make reviews exist": the route and the 20/hour
counter already accepted `kind: 'review'`, but no code ever wrote a `trek_reviews` row.
A "Write a review" toggle on `/trek/[id]` now appears for a trekker with a confirmed,
finished booking who has not reviewed yet; `submitReview()` uploads through the route
and inserts under the `0018` gate. `0031` restates the form's five-photo cap as a CHECK
(**not applied**). The Part-3 storage cleanup migration becomes `0032`. Detail in §2.

## Uploads to Cloudflare R2 — Part 1 written and green, nothing live  ·  2026-09-21

The three browser-side `supabase.storage` uploads become one `POST /api/upload` route
handler that sniffs the real file type, names the object itself, calls the same company
RPCs the storage policies called, and asks a new `record_upload()` RPC (`0030`, additive,
**not applied**) before each signed `aws4fetch` PUT. `uploadImage()` replaces the inline
blocks, `uploadErrorMessage()` becomes a plain map, `sanitizeFileName()` is deleted,
the five default-image literals move to `src/lib/defaultImages.ts`, `/api/*` gets a JSON
401 in the middleware, and both hosts sit in `remotePatterns` / `img-src` until Part 3.
The copy script and the URL rewrite for Parts 2/3 are written. 13 unit + 9 DB tests
added; the storage policy tests stay until `0031` removes what they assert. The R2
bucket does not exist yet — the Cloudflare MCP token cannot create one — so the
dashboard steps and everything after are row #0 in §1.0. Detail in §2.
...
is that follow-up — **applied and verified live 2026-09-05 12:01:25+00** (ledger
`0001`–`0015`; all six constraints `convalidated`; both regexes re-evaluated on
the live server, including the `\d`/`\s` ARE check `0013` made mandatory —
`'dsdsds'` and `'\d\s'` both fail the phone rule, and `[:space:]` catches
`'a b@example.com'` in the email one). Same class as `0011`: no abuse story, just a rule living in
...
`0014_pin-company-website-to-an-http-scheme.sql` — **applied and verified live
2026-09-05 11:50:34+00** (ledger `0001`–`0014`, no gaps; `companies_website_scheme`
read back from `pg_constraint`, `convalidated`; the pattern re-evaluated on the
live server, where `javascript:`, `data:`, `vbscript:`, `//evil.example`, `ftp://`
and `''` are rejected and `https://`, `http://` and `HTTPS://` accepted).
