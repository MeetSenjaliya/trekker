...
> Snapshot date: 2026-06-13; multi-tenant changes folded in 2026-07-02. Supabase project: `dtjmyqogeozrzzbdjokr`.
...
Since 2026-07-02 it is a **multi-tenant marketplace**: independent trek **companies** apply self-serve (`/company/apply` → `apply_for_company()` RPC, lands `pending`), get approved by a manually-appointed **platform admin**, and own their treks (`treks.company_id`, soft-delete via `is_active`). Full design: [MULTI_TENANT_PLAN.md](MULTI_TENANT_PLAN.md). DB is done (Phase A) and the app-layer application flow + guard layouts are in (Phase B); the `/dashboard` and `/admin` page sets and public `/company/[slug]` storefronts are Phases C–E, not built yet.
...
│   ├── dashboard/layout.tsx  # Guard: requires company membership (redirects to /company/apply); pages = Phase C
│   ├── admin/layout.tsx      # Guard: requires is_platform_admin() (redirects to /); pages = Phase D
...
| `/company/apply` | public | Company application form; prompts login client-side (`useAuth()`), the RPC itself requires auth. `/company/*` reserved for Phase E storefronts |
| `/dashboard/*` | **protected + company members only** | Guard layout only for now (redirects non-members to `/company/apply`, banners non-approved companies); pages are Phase C |
| `/admin/*` | **protected + platform admins only** | Guard layout only for now (`is_platform_admin()` RPC else `/`); pages are Phase D |
...
| `company-logos` | company logo/cover (Phase C UI) | `{company_id}/file` — writes scoped to company membership |
| `trek-images` | trek cover/photos (Phase C UI) | `{company_id}/{trek_id}/file` — writes scoped to company membership |

All buckets are public-read; writes are owner-scoped (M1 fix) or company-scoped (multi-tenant). See DATABASE.md §9 for the listing-advisor caveat.
...
Database-level (see DATABASE.md §11 for detail): ~~broken `trg_initial_trek_message`~~ (dropped 2026-07-02 by the multi-tenant migration — trek creation works now), duplicate dead notification triggers, `platform_admins` still **empty** (run `supabase/phases/phase-d-platform-admin.sql` before anything admin-side works), plus open advisors (security_definer_view on `public_profiles`, public-bucket listing incl. the two new company buckets, RPC-exposed SECURITY DEFINER functions incl. the new company RPCs — all fail safely via internal auth checks, leaked-password protection off, Postgres patch pending).
