...
### Join a trek (+ chat)
`src/lib/joinTrek.ts → joinTrekBatchAndChat(userId, trekId, trekTitle, date)` calls the RPC `join_trek_and_chat(p_user_id, p_trek_id, p_batch_date)`. The RPC (SECURITY DEFINER) atomically: upserts the `trek_batches` row, upserts the batch `conversation`, inserts the `trek_participants` row, and adds the user to `conversation_participants`. It derives the caller from `auth.uid()` and refuses `p_user_id` mismatches. Returns `{ batch_id, participant_id, conversation_id }`. Before any of that it refuses company accounts (`is_trekker()`), and then treks that are archived or whose company is no longer approved (`is_trek_bookable()`) — the second refusal added by **`0021`, applied and verified live 2026-09-09 12:55:52+00**. The `trek_participants` INSERT policy carries the same two checks, because a direct `POST /rest/v1/trek_participants` skips the RPC entirely. What that policy never checked was `status`: a direct POST could write `'confirmed'` on a full departure (or send nothing and get the default). **`0024`** (**applied and verified live 2026-09-14 09:15:46+00**) decides it in a BEFORE INSERT trigger, `assign_participant_status()` — batch locked, confirmed seats counted, `NEW.status` overwritten — so both paths land the same answer, and the RPC now reads the status back from `RETURNING` instead of computing its own.
...
- Security headers ship from `next.config.mjs` (2026-08-12); the **CSP is enforcing** as of 2026-09-01 (`CSP_ENFORCE=1` in Vercel, Production + Preview). The header name is picked at build time, so the flag only takes effect on a rebuild, and hashed per-deploy URLs keep serving whatever they were built with — verify on the alias. No app-level rate limiting; verbose `console.error(JSON.stringify(error))` can leak DB detail.

Database-level (see DATABASE.md §11 for detail): ~~broken `trg_initial_trek_message`~~ (dropped 2026-07-02 by the multi-tenant migration — trek creation works now), ~~`platform_admins` empty~~ (populated 2026-07-02), ~~duplicate dead notification triggers on `trek_participants` (three fire on join — `trek-join-notification`, `trek_join_email_trigger`, plus `trek-leave-notification` on delete)~~ (dropped 2026-08-26 in `0007` — `trek_join_email_trigger`/`trek_remove_email_trigger` and their functions posted to an edge fn that was never deployed).
...
| `auth_leaked_password_protection` disabled | 1 WARN | Pro-only toggle. **Decided 2026-09-15: not upgrading** — `isPasswordPwned()` in `src/lib/auth.ts` is the accepted control (FEATURES.md §2). Expected WARN, not actionable |
| `vulnerable_postgres_version` (`supabase-postgres-17.4.1.069`) | 1 WARN → **closed 2026-09-15** | Upgraded in place from the dashboard (Project Settings → Infrastructure). Re-read the advisor to confirm the WARN is gone — recorded while the MCP server was unreachable |
...
`npm test` runs two Vitest projects (`vitest.config.mts`):
