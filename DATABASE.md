...
| `comment` | text | |
| `created_at` | timestamptz | `now()` |
| `photo_urls` | text[] | default `{}`; `CHECK cardinality(photo_urls) <= 5` (`trek_reviews_photo_urls_max`, `0031` — **not yet applied**; the form's cap, restated for direct API writes) |
| `trek_date` | date | |
...
| `record_upload(text)` | boolean | **DEFINER** | pinned | **Added 2026-09-21 (`0030`, applied 2026-09-21 08:43 UTC — ledger row read over MCP).** The upload cap as the Cloudflare R2 route ([src/app/api/upload/route.ts](src/app/api/upload/route.ts)) sees it — called **before** the R2 PUT. Takes a per-(action, user) advisory lock, counts the trailing hour in `rate_events` against `storage_rate_rule(p_bucket)`, then either inserts the row and returns `true` or returns `false` (→ the route's 429). No session or uncovered bucket → `false`, nothing written. EXECUTE: `authenticated` only. Replaces both `enforce_storage_rate_limit()` and `upload_rate_limited()` once `0031` retires them. |
